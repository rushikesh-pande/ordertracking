package com.ordertracking.db.config;
 ...
 *
 * Usage in service:
 *   @Cacheable(value = "ordertrackingCache", key = "#id")
 *   @CacheEvict(value = "ordertrackingCache", key = "#id")
 *   @CachePut(value  = "ordertrackingCache", key = "#result.id")
 */
@Configuration
@EnableCaching
public class CacheConfig {
    ...
    public CacheManager cacheManager(RedisConnectionFactory factory) {
        ...
        // ── ordertracking specific caches ─────────────────────────────────────
        caches.put("ordertrackingCache",       defaultConfig().entryTtl(MEDIUM));
        caches.put("ordertrackingListCache",   defaultConfig().entryTtl(SHORT));
        caches.put("ordertrackingCountCache",  defaultConfig().entryTtl(SHORT));
        caches.put("ordertrackingRefCache",    defaultConfig().entryTtl(LONG));
