...
-- CREATE INDEX IF NOT EXISTS idx_ordertracking_status ON ordertrackings (status);

-- Created date (pagination/sorting by date)
-- CREATE INDEX IF NOT EXISTS idx_ordertracking_created_at ON ordertrackings (created_at DESC);

-- Composite: status + created_at (covers most list queries)
-- CREATE INDEX IF NOT EXISTS idx_ordertracking_status_created
--     ON ordertrackings (status, created_at DESC);

-- Foreign key index (prevents full-table scan on joins)
-- CREATE INDEX IF NOT EXISTS idx_ordertracking_user_id ON ordertrackings (user_id);
